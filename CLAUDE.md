...
7. **Mobile-first, older eyes.** Base font 18px, nothing below 15px except tracked all-caps labels at 14px, touch targets ≥ 56px, dark by default, high contrast, `touch-action: manipulation` on everything, `prefers-reduced-motion` respected. Drive taps from `pointerup`, not `click`.
